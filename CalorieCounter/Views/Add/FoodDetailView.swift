...
struct FoodDetailView: View {
    ...
    init(item: APIFoodItem,
        ...
        self.onAdd = onAdd
        _grams = State(initialValue: item.servingSize ?? 100)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    HStack {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Stepper("", value: $grams, in: 1...1500, step: 5).labelsHidden()
                        }
                    }
                ...
                // Skálázott értékek
                HStack(spacing: 12) {
                    StatBox(title: "Calories", value: scaledKcal.map { "\($0) kcal" } ?? "—")
                    StatBox(title: "Protein",  value: scaledProtein.map { String(format: "%.1f g", $0) } ?? "—")
                    StatBox(title: "Carbs",    value: scaledCarbs.map   { String(format: "%.1f g", $0) } ?? "—")
                    StatBox(title: "Fat",      value: scaledFat.map     { String(format: "%.1f g", $0) } ?? "—")
                }
                    ...
                    .sheet(isPresented: $showPicker) {
                        VStack {
                            DatePicker("Date", selection: $date, displayedComponents: .date)
                                ...
                                .padding()
                            Button("Done") { showPicker = false }.padding(.bottom, 12)
                        }
                ...
                // hagyjunk helyet az alsó gombnak
                Spacer(minLength: 8)
            }
            .padding(16)
        ...
        .safeAreaInset(edge: .bottom) {
            ...
            .background(.ultraThinMaterial) // finom elválasztás
        }
    }
}
            ...
            if let url {
                AsyncImage(url: url) { phase in
                    ...
                    case .success(let img):
                        img.resizable()
                            .scaledToFit()        // <<< fontos: teljes kép, 4 sarok látszik
                            .padding(8)           // kicsi margó a kártyán belül
                    case .empty:
                        ProgressView()
                    default:
                        Image(systemName: "photo").font(.largeTitle).opacity(0.3)
                    }
                }
